...
st.set_page_config(
    page_title="AI Director Master Shot-List Studio", layout="wide"
)
...
if os.path.exists(image_file):
  ...
  bg_img_style = (
      f"background-image: url('data:image/jpeg;base64,{bin_str}');"
  )
else:
  bg_img_style = "background-image:"
  " url('https://w0.peakpx.com/wallpaper/705/104/HD-wallpaper-anime-girls-playing-games-bed-short-hair-blond.jpg');"
...
<style>
    ...
    .scene-box {{ background-color: rgba(255, 255, 255, 0.95); border-left: 5px solid #1e40af; padding: 15px; border-radius: 8px; color: #0f172a; margin-bottom: 15px; }}
</style>
"""
...
def generate_text_content(client, prompt_text):
  ...
  target_list = (
      priority_models
      if priority_models
      else ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-2.5-flash-lite"]
  )
...
if "extracted_scenes" not in st.session_state:
  st.session_state.extracted_scenes = []
if "scene_boards" not in st.session_state:
...
duration_min = col_min.number_input(
    "Minutes", min_value=0, max_value=40, value=10
)
duration_sec = col_sec.number_input(
    "Seconds", min_value=0, max_value=59, value=0
)
...
secondary_type = st.sidebar.selectbox(
    "Genre 2",
    ["None", "Action", "Drama", "Thriller", "Comedy", "Romance", "Mystery"],
)
art_style = st.sidebar.selectbox(
    "Style",
    [
        "Japan Animation Style (Anime)",
        "3D Disney Cartoon Style",
        "Realistic Cinematic Movie",
        "Cyberpunk Art",
    ],
)
...
if st.session_state.story_stage == "input":
  ...
  if st.button("Step 1: Brainstorm Master Screenplay"):
    if not user_api_key:
      ...
      st.error("ကျေးဇူးပြု၍ အချိန်သတ်မှတ်ပေးပါ။")
    else:
      try:
        ...
        combo_genre = (
            ...
            else f"{story_type} + {secondary_type}"
        )

        if total_target_seconds <= 60:
          length_instruction = (
              "SHORT SCREENPLAY (1-2 distinct scenes, around 300 words)."
          )
        elif total_target_seconds <= 300:
          length_instruction = (
              "MEDIUM SCREENPLAY (3-4 structured scenes, around 1000 words)."
          )
        else:
          length_instruction = (
              "EPIC DETAILED LONG SCREENPLAY (Strictly 8 to 12 distinct scenes,"
              " around 2500+ words). Write comprehensive deep dialogues,"
              " elaborate scene descriptions, emotion, and character actions"
              " for each scene. DO NOT summarize or skip events."
          )

        while attempt < max_attempts and not passed_gate:
          attempt += 1
          status_box.markdown(
              f"🔄 **Screenplay Generation: Loop {attempt}/{max_attempts}...**"
          )
          ...
          try:
            ...
            ) in ["RECITATION", "SAFETY", "8"]:
              st.error(
                  f"⚠️ Loop {attempt}: Gemini Safety Blocked. Retrying next"
                  " loop..."
              )
              continue
...
if st.session_state.story_stage in ["story_ready", "scenes_extracted"]:
  st.markdown(
      "<h3 style='color: white;'>📖 Approved Screenplay Script</h3>",
      unsafe_allow_html=True,
  )
  st.text_area(
      "Story View",
      value=st.session_state.approved_story,
      height=350,
  ...
  if st.session_state.story_stage == "story_ready":
    if st.button("Separate Screenplay Into Scene Chunks"):
      try:
        client = get_genai_client(user_api_key)
        chunk_command = (
            "Break this script into logical individual scenes using format"
            " SCENE_BLOCK_START Scene X: Description Content: Text"
            f" SCENE_BLOCK_END. Script: {st.session_state.approved_story}"
        )
        res, _ = generate_text_content(client, chunk_command)
        raw_scenes = re.findall(
            r"SCENE_BLOCK_START(.*?)SCENE_BLOCK_END",
            res.text,
            flags=re.DOTALL,
        )
  ...
  if st.session_state.story_stage == "scenes_extracted":
    if "Disney" in art_style:
      mj_style = (
          "3D Pixar Disney Animation Style, Vibrant Clay Render, Raytracing"
      )
      ...
      mj_style = (
          "Cinematic Still, Film Grain, 8k Resolution, Photorealistic, --style"
          " raw --v 6.0"
      )
    ...
    for idx, scene in enumerate(st.session_state.extracted_scenes):
      ...
      with st.container():
        st.markdown(
            f"<div class='scene-box'><h4>📌 {scene['title']}</h4><p>{scene['content']}</p></div>",
            unsafe_allow_html=True,
        )
            ...
            try:
              ...
              if is_scene_one:
                ...
                char_sheet_instruction = (
                    "Do NOT generate any Character Profiles or Model Sheets"
                    " here. Start directly with the Shot List Breakdown."
                )
                structure_format = ""
              ...
              shot_command = f"""
                            ...
                            🎨 Image Prompt (Midjourney): [MUST start with the Camera Framing/Angle keyword, e.g., 'An extreme wide shot establishing shot of...', 'A close up shot of...']. Describe the environment and character states clearly following style: {mj_style} --ar {image_ratio}
                            
                            👥 DIALOGUE / NARRATION: [Character Name or N/A]: "[Script line or narration text translated to {story_language}]"
                            
                            🎥 Video Prompt & Direction (Runway/Luma): [Combine Camera Movement like Pan/Zoom/Tilt with the characters' physical action actions], Motion Style: {v_style}
              ...
              with st.spinner(
                  f"{scene['title']} အတွက် Prompts များ ထုတ်လုပ်နေသည်..."
              ):
        ...
        with col2:
          if idx in st.session_state.scene_boards:
            st.text_area(
                "Shot Output",
                value=st.session_state.scene_boards[idx],
                height=300,
                key=f"text_{idx}",
            )
            st.download_button(
...
# Main Content Div ကို အပြီးသတ် ပိတ်လိုက်ခြင်း
