...
if st.button("Generate Production Board"):
    if not user_api_key:
        ...
        try:
            ...
            if get_image_prompt:
                # 🌅 Image Prompt Control Upgraded (Background, Environment, Atmosphere added)
            ...
            if get_video_prompt:
                # 🎥 Video Prompt Control Upgraded (Background Motion & Camera Paths added)
...
st.markdown("</div>", unsafe_allow_html=True)        .stTextInput > div > div > input::placeholder {
            color: #475569 !important;
            opacity: 1 !important;
        }
        .stTextInput > div > div > input:focus {
